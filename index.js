...
var cols = 106-1; // 리스트 표현위해 -1
var rows = 17-1;//위와같음,여기중요! 17역할 여기임
var value = prompt('Tupper\'s self-referential formula에서 17대신 대입할수아무글자도안쓰면 17로 대입')
if(value !== ''){
    rows = value-1
}
var gsize = 7;
var grid = (cols + 1) * (rows + 1);
...
var getDecimalFromMap = function () {
    ...
    setDecString(decimal.times(rows+1));
    setPresetUrl();
}
...
$('#bitArea').keyup(function() {
    ...
    if(!/^[0-1]*$/.test(input) && input != "") bitError.text("Not a binary number.");
        ...
        bitError.text("");
        setBitString(input);
        setDecString(new BigNumber(input.padEnd(1802, 0), 2).times(rows+1));
        setBitMap();
        setPresetUrl();
    } 
});
...
$('#decArea').keyup(function() {
    ...
    if(!/^\d+$/.test(input) && input != "") decError.text("Not a positive number.");
        ...
        decError.text("");
        setBitString(bigInput.dividedBy(rows+1).integerValue(BigNumber.ROUND_FLOOR).toString(2).padStart(1802, 0));
...
$("#presets button").click(function() {
    ...
    // decimal = big;
    setBitString("0".repeat(parseInt(b.attr('p'))) + big.dividedBy(rows+1).integerValue(BigNumber.ROUND_FLOOR).toString(2));
    setBitMap();
    setPresetUrl();
});
...
$("#presets #netpbm").change(function(evt) {
    ...
    reader.onload = function(e){
        try {
            ...
            decError.text("");
            setDecString(new BigNumber(bitString, 2).times(rows+1));
...
function setPresetUrl() {
    ...
    BigNumber.config({ ALPHABET: '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ' });
    const base62String = decimal.dividedBy(rows+1).toString(62);
    if (base62String !== '0') {
